...
float perlinNoise(float3 pos)
{
    ...
    float res = lerp(w,
        lerp(v,
            lerp(u, grad(hash(AA, 0, 0), x, y, z), grad(hash(BA, 0, 0), x - 1, y, z)),
            lerp(u, grad(hash(AB, 0, 0), x, y - 1, z), grad(hash(BB, 0, 0), x - 1, y - 1, z))
        ),
        lerp(v,
            lerp(u, grad(hash(AA, 0, 1), x, y, z - 1), grad(hash(BA, 0, 1), x - 1, y, z - 1)),
            lerp(u, grad(hash(AB, 0, 1), x, y - 1, z - 1), grad(hash(BB, 0, 1), x - 1, y - 1, z - 1))
        )
    );
